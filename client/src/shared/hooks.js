...
export const mqBetween = (breakpointSmall, breakpointLarge) =>
  `screen and (min-width: ${breakpointSmall}px) and (max-width: ${breakpointLarge}px)`;
...
export const useClickOutside = (ref, callback) => {
  useEffect(() => {
    const handleClick = (evt) => {
      ...
      if (
        refs.every(
          (refItem) => refItem.current && !refItem.current.contains(evt.target)
        )
      ) {
        callback();
      }
